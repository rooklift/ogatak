...
let fullbox_prototype = {
	...
	display_perf_report: function(all_stats) {
		...
		for (let key of ["B", "W"]) {
			...
			lines.push(`          Mean point loss: ${span_string} ${(stats.points_lost / stats.moves_analysed).toFixed(2)}</span>`);
			span_string = winners.top1 === key ? `<span class="green">` : "<span>";
			lines.push(`              AI top move: ${span_string} ${(stats.top1 * 100 / stats.moves_analysed).toFixed(1)}%</span>`);
			...
			span_string = winners.top5 === key ? `<span class="green">` : "<span>";
			lines.push(`                 AI top 5: ${span_string} ${(stats.top5 * 100 / stats.moves_analysed).toFixed(1)}%</span>`);
			lines.push("");
			lines.push("");
		}
